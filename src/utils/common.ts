...
import {BlockEventName, Chart, CHARTS, ParsedEventsData, ParsedEventsDataMap, TrackingMetrics} from './types'
import {BigDecimal} from '@subsquid/big-decimal'

const chainConfig = getChain()

export async function updateChart<T extends Chart>(
    ...
    const chartTimes = getTimePoints(lastChart.timestamp, current.lastUpdate, intervalAccumultor)
    if (chartTimes.length > 0) {
        ...
        await ctx.store.upsert(newStats as any)
...
export class ChartsCache {
    ...
    static readonly setLatest = function <T extends Chart>(entityClass: EntityClass<T>, entity: T): void {
        ChartsCache.cache.set(entityClass, entity)
    }

    static readonly getLatest = function <T extends Chart>(entityClass: EntityClass<T>) {
        return assertNotNull(ChartsCache.cache.get(entityClass), 'You have to initialize ChartCache before using') as T
    }

    static readonly init = async function (ctx: ProcessorContext<StoreWithCache>): Promise<void> {
        await Promise.all(
...
export function getTimePoints(lastDate: Date, curDate: Date, intervalAccumultor: (i: Date) => Date) {
...
export function fillStats<T extends Chart>(lastState: T, currentStats: Current, timePoints: Date[]) {
    ...
    for (const stat in CURRENT_INIT) {
        //@ts-ignore
        const change = BigDecimal(currentStats[stat] ?? 0).sub(lastState[stat] ?? 0)
        //@ts-ignore
        newStates[0][`${stat}Change`] = typeof CURRENT_INIT[stat] === 'number' ? BigDecimal(change).toNumber() : BigInt(BigDecimal(change).toString())
        //@ts-ignore
        newStates[0][`${stat}ChangeRatio`] = lastState[stat]
            ? //@ts-ignore
            change.div(lastState[stat]).mul(100).toNumber()
            : 0
    }
    return newStates
}
...
export class ParsedEventsDataScope {
    ...
    set(section: BlockEventName, value: ParsedEventsData) {
        this.scope.set(section, (this.scope.get(section) || new Map()).set(value.id, value))
    }
...
export function encodeAccount(id: Uint8Array, prefix?: string | number | undefined) {
...
export async function getOrCreateBackupCounter(ctx: ProcessorContext<StoreWithCache>) {
    ...
    if (counterCahced) {
        ...
        await ctx.store.upsert(counterNew)
        return counterNew
    }
}

export async function getOrCreateCurrent(ctx: ProcessorContext<StoreWithCache>) {
    const currentCached = await ctx.store.get(Current, '1')
    if (currentCached) {
        ...
        await ctx.store.upsert(currentNew)
        return currentNew
    }
}
